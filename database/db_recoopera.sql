...
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL'; -- Define o modo SQL como 'TRADITIONAL', que é mais restrito. Ele transforma avisos em erros (ex: inserir um dado do tipo errado), prevenindo comportamentos inesperados.
...
CREATE TABLE IF NOT EXISTS `usuarios` (
  ...
  -- Um índice único garante que não existam dois usuários com o mesmo e-mail, mantendo a integridade dos dados.
  -- Também acelera muito as buscas por e-mail, como no momento do login.
  UNIQUE INDEX `uidx_email` (`email`)
);

-- Tabela para armazenar tokens temporários usados para validação de e-mail e recuperação de senha.
CREATE TABLE IF NOT EXISTS `tokens_validacao` (
  ...
  -- Define a relação entre o token e o usuário.
  -- ON DELETE CASCADE: Se um usuário for deletado, todos os seus tokens também serão.
...
CREATE TABLE IF NOT EXISTS `cooperativas` (
  ...
  INDEX `idx_cidade_estado` (`cidade`, `estado`), -- Índice para otimizar buscas por localização.
...
-- Tabela para armazenar os documentos enviados pelas cooperativas para validação.
...
-- Catálogo central com os nomes padronizados dos materiais.
...
-- Tabela para permitir que cooperativas associem nomes regionais (apelidos) a um material padrão do catálogo.
...
CREATE TABLE IF NOT EXISTS `vendas` (
  ...
  CONSTRAINT `fk_vendas_cooperativas`
    FOREIGN KEY (`id_cooperativa`)
    REFERENCES `cooperativas` (`id_cooperativa`)
    ON DELETE RESTRICT ON UPDATE CASCADE, -- Impede que uma cooperativa ou comprador seja deletado se houver vendas associadas a eles, garantindo a integridade do histórico.
...
-- Tabela de tags pré-definidas para facilitar o feedback.
...
CREATE TABLE IF NOT EXISTS `avaliacao_feedback_selecionado` (
  ...
  PRIMARY KEY (`id_avaliacao`, `id_feedback_tag`), -- Chave primária composta para garantir a unicidade da relação.
...
-- É preenchida por um procedimento agendado para evitar cálculos pesados em tempo real.
...
CREATE TABLE IF NOT EXISTS `historico_score` (
  ...
  `detalhe_json` JSON NULL, -- Campo para guardar o contexto do cálculo (ex: qual avaliação o disparou).
...
-- Permite alterar regras de negócio (como os pesos do score) sem precisar alterar o código-fonte.
...
CREATE TABLE IF NOT EXISTS `audit_log` (
  ...
  `detalhes_antigos_json` JSON NULL, -- Guarda o estado do registro antes da alteração.
  `detalhes_novos_json` JSON NULL,   -- Guarda o estado do registro depois da alteração.
...
BEGIN
    ...
    -- Carrega os parâmetros da tabela de configuração para as variáveis.
    ...
    -- Conta o número total de avaliações do comprador.
    ...
    -- Caso base: se o comprador não tem avaliações, retorna a nota neutra padrão.
    IF v_num_avaliacoes = 0 THEN
        RETURN v_aval_neutra;
    END IF;

    -- Calcula a média ponderada das notas, levando em conta os pesos de cada critério e o "peso temporal" (notas mais recentes valem mais).
    SELECT
    ...
    -- Calcula o score bruto (média ponderada).
    ...
    -- Isso evita que compradores com poucas avaliações tenham scores extremos (muito altos ou muito baixos).
...
-- Sua função é recalcular o valor total da venda e atualizar a tabela `vendas`.
...
BEGIN
    ...
    SET v.valor_total = (
        ...
        WHERE vi.id_venda = NEW.id_venda -- NEW se refere à nova linha que foi inserida.
    )
...
BEGIN
    ...
    SET v.valor_total = (
        SELECT COALESCE(SUM(vi.quantidade_kg * vi.preco_por_kg), 0)
        FROM vendas_itens vi
        WHERE vi.id_venda = OLD.id_venda -- OLD se refere à linha que foi removida.
    )
...
BEGIN
    ...
    -- Descobre qual comprador está sendo avaliado através da venda.
    ...
    -- Conta o novo total de avaliações.
    SELECT COUNT(*) INTO v_num_avaliacoes
    FROM avaliacoes_compradores ac
    JOIN vendas v ON ac.id_venda = v.id_venda
    WHERE v.id_comprador = v_id_comprador;

    -- Atualiza a tabela de compradores com o novo score e a contagem de avaliações.
    UPDATE compradores
    SET score_confianca = v_novo_score,
        numero_avaliacoes = v_num_avaliacoes
    WHERE id_comprador = v_id_comprador;

    -- Insere um registro no histórico de score para auditoria.
...
-- Ele pré-calcula as estatísticas de preços e as armazena na tabela `precos_regionais`.
...
BEGIN
    ...
    -- Insere os dados calculados na tabela.
...
-- Evento agendado que executa o procedimento de materialização de preços.
...
-- View que expõe os dados públicos dos compradores de forma segura.
...
-- View que fornece dados de forma anônima
...
-- Concede permissões específicas e limitadas para o papel 'cooperativa_role'.
-- Uma cooperativa só pode ver dados (SELECT), nunca alterar ou deletar dados de outras cooperativas.
...
-- Permite que a cooperativa insira registros de suas próprias atividades.
...
-- Permite que cooperativas adicionem e atualizem informações de compradores.
...
-- Permite que o papel cooperativa execute a função de cálculo de score.
GRANT EXECUTE ON FUNCTION `recoopera`.`calcular_score_confianca` TO 'cooperativa_role';

-- Restaura as configurações originais do MySQL para o modo de operação normal.
